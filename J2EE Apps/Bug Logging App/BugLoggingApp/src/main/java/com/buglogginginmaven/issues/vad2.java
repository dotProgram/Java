...
{    
           ...
           // admin_id_or_admin_id_of_the_logged_in_user stores the Admin_ID of current logged client
           int admin_id_or_admin_id_of_the_logged_in_user;
...
//res.sendRedirect("projects/project.jsp");
      ...
      {  
                   ...
                   //     int admin_id_of_the_logged_in_user = (int)session.getAttribute("admin_id_of_the_logged_in_user");
                        System.out.println("----------------------------------------------------------------------------------------------------------------");  
                        System.out.println("Admin_id_of_the_logged_in_user: "+session.getAttribute("admin_id_of_the_logged_in_user"));  
                        System.out.println("Session Variable:  id = "+session.getAttribute("id")); 
                        System.out.println("Session Variable:  logged_user_id = "+session.getAttribute("logged_user_id")); 
                        System.out.println("Session Variable:  logged_user_id_in_session = "+session.getAttribute("logged_user_id_in_session")); 
                        
                        if (session.getAttribute("role").equals("admin")){
                                    System.out.println("Loggd in Client is Admin");  
                                    //if the role is Admin, then directly fetch the Admin Id from Session
                                    this.admin_id_or_admin_id_of_the_logged_in_user =(int) session.getAttribute("id");
                                    
                                    System.out.println("Admin Id: "+this.admin_id_or_admin_id_of_the_logged_in_user);  
                        }
                        else{
                                    System.out.println("Loggd in Client is User");  
                                    //if the role is Admin, then retrieving the  Admin Id of the current user from Session

                                    this.admin_id_or_admin_id_of_the_logged_in_user =Integer.parseInt((String)session.getAttribute("admin_id_of_the_logged_in_user")) ;
                                    System.out.println("Admin Id: "+this.admin_id_or_admin_id_of_the_logged_in_user);  
                        ...
                        int id = 000;
                                               ...
                                               if( id == 0){
                                                            System.out.println("<h1>Handeling the Request where ID = 0</h1><br>");  
       ...
       //     out.println("</p> </div> </div> <div style=\"height: 100vh\"></div> <div class=\"card mb-4\"><div class=\"card-body\">When scrolling, the navigation stays at the top of the page. This is the end of the static navigation demo.</div></div> </div> </main> <footer class=\"py-4 bg-light mt-auto\"> <div class=\"container-fluid\"> <div class=\"d-flex align-items-center justify-content-between small\"> <div class=\"text-muted\">Copyright &copy; Your Website 2020</div> <div> <a href=\"#\">Privacy Policy</a> &middot; <a href=\"#\">Terms &amp; Conditions</a> </div> </div> </div> </footer> </div> </div> <script src=\"https://code.jquery.com/jquery-3.5.1.slim.min.js\" crossorigin=\"anonymous\"></script> <script src=\"https://cdn.jsdelivr.net/npm/bootstrap@4.5.3/dist/js/bootstrap.bundle.min.js\" crossorigin=\"anonymous\"></script> <script src=\"../js/scripts.js\"></script> </body> </html>");                                                
          ...
          private String  processViewListRequest(PrintWriter out){
                                                    ...
                                                    {  
                                                                      ...
                                                                      //  ResultSet rs = stmt.executeQuery("select * from logged_issue");  SELECT * FROM logged_issue  WHERE project_id IN (SELECT project_uid FROM project_admin_relationship WHERE project_adminowner = 146); 
                                                                       ResultSet rs = stmt.executeQuery("SELECT * FROM logged_issue  WHERE project_id IN (SELECT project_uid FROM project_admin_relationship WHERE project_adminowner = "+this.admin_id_or_admin_id_of_the_logged_in_user+")"); 
                                                                        ...
                                                                        out.println("<tr><th>Select</th><th>issue title</th><th>Logged By</th><th>assigned user</th><th>issue status</th><th>Platform</th><th>Platform version</th><tr>");  
                                                                                ...
                                                                                {  
                                                                                  ...
                                                                                  //out.println("<tr style=\"cursor: pointer;\"  onclick=\"window.location='vad2?id="+id+"&request=detail'\" data-href=\"vad\"><td><input type=checkbox id=check name=checkbox value="+ issue_title +"></td><td>" + issue_title + "</td><td>" + issue_status + "</td><td>" + issue_status + "</td><td>" + issue_status + "</td><td>" + platform + "</td><td>" + platform_version + "</td><td><input type=\"submit\" value=\"Delete\" name=\"delete\"/></td></tr>");   
                                                                                }  
